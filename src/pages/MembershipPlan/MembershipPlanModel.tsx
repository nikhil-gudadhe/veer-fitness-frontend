...
import { createMembershipPlan, updateMembershipPlan } from '../../../store/Slices/membershipPlanSlice';

interface MembershipPlanModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (data: MembershipPlanFormInputs) => void;
    membershipPlan?: MembershipPlanFormInputs | null;
}

const MembershipPlanModel: React.FC<MembershipPlanModalProps> = ({ isOpen, onClose, membershipPlan }) => {
    const { register, handleSubmit, reset, formState: { errors } } = useForm<MembershipPlanFormInputs>({
    ...
    }, [membershipPlan, reset]);

    const onSubmit: SubmitHandler<MembershipPlanFormInputs> = (data) => {
        if (data._id) {
            ...
            <div className="relative m-auto w-full max-w-180 rounded-sm border border-stroke bg-gray p-4 shadow-default dark:border-strokedark dark:bg-meta-4 sm:p-8 xl:p-10">
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className="mb-4.5">
                        ...
                        />
                        {errors.name && <span className="text-red-500">Plan name is required</span>}
                    </div>

                    <div className="mb-6">
                        <label className="mb-2.5 block text-black dark:text-white">Plan Description</label>
                        <textarea
                            {...register('description')}
                            rows={6}
                            placeholder="Enter description for membership plan"
                            className="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                        ></textarea>
                    </div>

                    <div className="mb-5.5">
                        <label className="mb-2.5 block text-black dark:text-white">Plan Duration (Months)</label>
                    ...
                    <div className="mb-4.5">
                        <label className="mb-2.5 block text-black dark:text-white">Plan Pricing</label>
                        ...
                        {errors.price && <span className="text-red-500">Membership plan price is required</span>}
                    </div>
