...
const EnquiryList: React.FC<EnquiryListProps> = ({ onEdit }) => {
  ...
  const filteredRows = enquiries.filter(
    (enquiry) =>
      enquiry.fullName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      enquiry.mobile.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const tableData = {
    ...
    rows: paginatedRows.map((enquiry) => ({
      ...
      actions: (
        <div className="flex items-center space-x-3.5">
          <button onClick={() => onEdit(enquiry)} className="hover:text-primary">
